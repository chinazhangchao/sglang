"""The fast-image-processor device comes from the published device config."""
...
class TestFastImageProcessorDevice(CustomTestCase):
    ...
    def test_device_follows_published_base_gpu_id(self):
        self.assertEqual(self._device(_make(base_gpu_id=3)), "cuda:3")

    def test_sequential_engines_use_latest_published_device(self):
        first, second = _make(base_gpu_id=0), _make(base_gpu_id=5)
        self.assertEqual(self._device(first), "cuda:5")
        self.assertEqual(self._device(second), "cuda:5")

    def test_context_override_moves_the_device(self):
        ...
        self.assertEqual(self._device(processor), "cuda:7")
